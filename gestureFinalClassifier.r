FinalClassifier = function(surv, gep, sets, results, maxUse = 1000){ 
	...
	for(x in 1:repeats){
		...
		if(stratify == "YES"){
			training[x,1,1] <- summary(coxph(Surv(surv[,1],surv[,2])~surv[,3] + strata(surv[,4]) , data = surv, subset = class.ab[,top] == 1))$coef[2]
			training[x,1,2] <- summary(coxph(Surv(surv[,1],surv[,2])~surv[,3] + strata(surv[,4]) , data = surv, subset = class.ab[,top] == 1))$coef[5]
			training[x,1,3] <- summary(coxph(Surv(surv[,1],surv[,2])~surv[,3] + strata(surv[,4]) , data = surv.v, subset = class.d[,top] == 1))$coef[2]
			training[x,1,4] <- summary(coxph(Surv(surv[,1],surv[,2])~surv[,3] + strata(surv[,4]) , data = surv.v, subset = class.d[,top] == 1))$coef[5]
		}
		if(stratify == "NO"){
			training[x,1,1] <- summary(coxph(Surv(surv[,1],surv[,2])~surv[,3], data = surv, subset = class.ab[,top] == 1))$coef[2]
			training[x,1,2] <- summary(coxph(Surv(surv[,1],surv[,2])~surv[,3], data = surv, subset = class.ab[,top] == 1))$coef[5]
			training[x,1,3] <- summary(coxph(Surv(surv[,1],surv[,2])~surv[,3], data = surv.v, subset = class.d[,top] == 1))$coef[2]
			training[x,1,4] <- summary(coxph(Surv(surv[,1],surv[,2])~surv[,3], data = surv.v, subset = class.d[,top] == 1))$coef[5]
		}	
		...
		for(g in 2:maxUse){
			...
			for(i in unique(sort(surv.t$score, decreasing = FALSE))){
				...
				if(length(unique(surv.t[which(surv.t$solution == 1),3])) == 2 & length(unique(surv.t[which(surv.t$solution == 0),3])) == 2){
					if(stratify == "YES"){
						results.e[count,1] <- summary(coxph(Surv(surv[,1],surv[,2]) ~surv[,3] + strata(surv[,4]), data = surv.t, subset = solution == 1))$coef[2]	
						results.e[count,2] <- summary(coxph(Surv(surv[,1],surv[,2]) ~surv[,3] + strata(surv[,4]), data = surv.t, subset = solution == 1))$coef[5]
					}
					if(stratify == "NO"){
						results.e[count,1] <- summary(coxph(Surv(surv[,1],surv[,2]) ~surv[,3], data = surv.t, subset = solution == 1))$coef[2]	
						results.e[count,2] <- summary(coxph(Surv(surv[,1],surv[,2]) ~surv[,3], data = surv.t, subset = solution == 1))$coef[5]
					}	
			...
			cutoff <- results.e[which(results.e[,1] == min(results.e[which(results.e[,3] > 0.2),1]))[1], 4 ]
			...
			if(stratify == "YES"){
				training[x,g,1] <- summary(coxph(Surv(surv[,1],surv[,2])~surv[,3] + strata(surv[,4]), data = surv.t, subset = solution == 1))$coef[2]
				training[x,g,2] <- summary(coxph(Surv(surv[,1],surv[,2])~surv[,3] + strata(surv[,4]), data = surv.t, subset = solution == 1))$coef[5]
				training[x,g,3] <- summary(coxph(Surv(surv[,1],surv[,2])~surv[,3] + strata(surv[,4]), data = surv.v, subset = solution == 1))$coef[2]
				training[x,g,4] <- summary(coxph(Surv(surv[,1],surv[,2])~surv[,3] + strata(surv[,4]), data = surv.v, subset = solution == 1))$coef[5]
			}
			if(stratify == "NO"){
				training[x,g,1] <- summary(coxph(Surv(surv[,1],surv[,2])~surv[,3], data = surv.t, subset = solution == 1))$coef[2]
				training[x,g,2] <- summary(coxph(Surv(surv[,1],surv[,2])~surv[,3], data = surv.t, subset = solution == 1))$coef[5]
				training[x,g,3] <- summary(coxph(Surv(surv[,1],surv[,2])~surv[,3], data = surv.v, subset = solution == 1))$coef[2]
				training[x,g,4] <- summary(coxph(Surv(surv[,1],surv[,2])~surv[,3], data = surv.v, subset = solution == 1))$coef[5]
			}
